...
def worker(path, number):
    ...
    driver.task_consume(write_task, _HOST, _PORT,
                        topic=number, segment="task_testing")
...
def consumer(path, topics):
    ...
    driver.share_consume(write_shared, _HOST, _PORT,
                         topics=topics, segment="shared_testing")
...
def test_share_queue(tmp_path):
    ...
    p.start()
    time.sleep(1)
    driver.share_publish('1', _HOST, _PORT,
                         segment="shared_testing", topic='1')
    time.sleep(1)
    p.terminate()
    ...
    p2.start()
    time.sleep(1)
    driver.share_publish('2', _HOST, _PORT,
                         segment="shared_testing", topic='2')
    driver.share_publish('3', _HOST, _PORT,
                         segment="shared_testing", topic='2')
    time.sleep(1)
    p1.terminate()
    p2.terminate()
